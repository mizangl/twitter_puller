...
public class FourImageTweet extends BaseTweetView {

  private final ImageView mediaImage1;
  private final ImageView mediaImage2;
  private final ImageView mediaImage3;
  private final ImageView mediaImage4;
  private Target<Bitmap> mediaTarget1;
  private Target<Bitmap> mediaTarget2;
  private Target<Bitmap> mediaTarget3;
  private Target<Bitmap> mediaTarget4;

  public FourImageTweet(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public FourImageTweet(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    mediaImage4 = (ImageView) findViewById(R.id.media4);
  }

  @Override protected void inflate(@NonNull Context context) {
    LayoutInflater.from(context).inflate(R.layout.view_tweet_with_four_image, this, true);
  }
  ...
  protected int measureDecorate(int widthMeasureSpec, int widthUsed, int heightMeasureSpec,
    ...
    if (mediaImage4.getVisibility() != GONE) {
      measureChildWithMargins(mediaImage4, widthMeasureSpec, widthUsed, heightMeasureSpec,
          heightUsed);
    }
  ...
  @Override protected void layoutDecorated(int paddingStart, int currentTop, int contentWidth,
    ...
    if (mediaImage3.getVisibility() != GONE) {
      layoutView(mediaImage3, contentStart, currentTop, contentWidth / 2,
          mediaImage3.getMeasuredHeight() / 2);
    }

    if (mediaImage4.getVisibility() != GONE) {
      layoutView(mediaImage4, contentStart, middleTop, contentWidth / 2,
          mediaImage4.getMeasuredHeight() / 2);
    }
  }
  ...
  @Override protected void bindDecorated(@NonNull TweetModel model) {
    ...
    mediaTarget4 = GlideApp.with(getContext()).asBitmap().load(url3).into(mediaImage4);
  }
